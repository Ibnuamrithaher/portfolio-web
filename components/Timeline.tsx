    ...
    <section id="pengalaman" className="py-24 px-6 bg-[#FAFAFA] dark:bg-[#0B1120] transition-colors duration-300 overflow-hidden">
      <div className="max-w-4xl mx-auto">
        <AnimateIn direction="up">
          <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter mb-16">Work Experience & Leadership.</h2>
        </AnimateIn>
        <div className="ml-1">
            ...
            <AnimateIn key={idx} direction="up" delay={idx * 0.1}>
              ...
              <div className="flex flex-col md:flex-row md:justify-between md:items-baseline mb-4">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-tight">
                  {exp.role} <span className="text-slate-400 font-medium whitespace-nowrap">@ {exp.company}</span>
                </h3>
              ...
              <ul className="space-y-3 mt-4">
                {exp.desc && (
                  <li className="text-[14px] text-slate-500 dark:text-slate-400 flex items-start group">
                    ...
                    <span className="leading-relaxed flex-1">{exp.desc}</span>
                  </li>
                )}
                {/* @ts-ignore - Ignore if bullets type is inferred dynamically */}
                {exp.bullets && exp.bullets.length > 0 && exp.bullets.map((bullet, bIdx) => (
        ...
        <div className="mt-24">
          ...
          <AnimateIn direction="up" delay={0.2}>
            <OrgSlider organizations={[
              ...
              {
                role: "Sekretaris Departemen Internal",
              ...
              {
                role: "Sekretaris Divisi",
                org: "UKM PSHT Unmul",
                period: "2020-2021",
                icon: "🥋"
              },
              {
                role: "Ketua Komisi Keuangan",
                org: "DPM Unmul (Student Representative Council)",
                period: "2019",
                icon: "💼"
              },
              {
                role: "Staf Kerohanian",
